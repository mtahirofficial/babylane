...
// sgMail.setApiKey('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');
...
const MailController = {
    sendMail: async options => {
        ...
        // lesssecureapps setting in gmail
        // const transporter = nodemailer.createTransport({
        //     service: 'gmail',
        //     host: 'localhost',
        //     port: 465,
        //     auth: {
        //         user: 'hmtahirs1@gmail.com',
        //         pass: 'tvmrgibB24287_',
        //     }
        // });
    ...
    sendToActive: async (req, res) => {
        try {
            ...
            for (const shop_id in shops) {
                ...
                const response = await MailController.sendMailToCustomer(shops[shop_id], subject, html);
                if (response[shop_id]) {
                    const data = { shop_id: shop_id, sent_to: response[shop_id].envelope.to.toString(), subject, token, sent_at: new Date() }
    ...
    trackOpen: async (req, res) => {
        ...
        if (!isNaN(req.params.shop_id)) {
            opened = true
            await mail_tracking.update({ opened_at: new Date() }, { where: { token: req.params.token } })
        }
        res.json({
            opened
        })
    ...
    trackClick: async (req, res) => {
        let clicked = false
        if (!isNaN(req.params.shop_id)) {
            const shopData = await shop.findOne({ where: { shop_id: req.params.shop_id } })
            clicked = true
            await mail_tracking.update({ clicked_at: new Date() }, { where: { token: req.params.token } })
            const url = encodeURIComponent(`https://apps.shopify.com/${SHOPIFY_API_KEY}?reveal_new_review=true&utm_campaign=installed&st_source=admin&st_campaign=rate-app`)
            res.redirect(`https://${shopData.domain}/admin/app_store/redirect?url=${url}`)
        } else
            res.json({
                clicked
            })
            ...
            await shop
                ...
                .then(async shops => {
                    for (const shopData of shops) {
                        ...
                        var html = fs.readFileSync(__dirname + "/free_shipping.html", "utf8");
                        html = html
                        ...
                        const response = await MailController.sendMailToCustomer(shopData, subject, html);
    ...
    sendToNonActivePlans: async (req, res) => {
        try {
            const plans = await DatabaseController.findAll(payment, { status: { [Op.ne]: 'ACTIVE' } })
            const shopIds = plans.map(plan => plan.shop_id)
            const shops = await DatabaseController.findAll(
                shop,
                {
            ...
            html = html
                ...
                .replace(/{{EX_ORDER_IMG_SRC}}/g, hostLink + '/client/images/ex_order.png')
                .replace(/{{VENDOR_IMG_SRC}}/g, hostLink + '/client/images/vendor.png')
                .replace(/{{YALIDINE_IMG_SRC}}/g, hostLink + '/client/images/yalidine.png')
            ...
            for (const s of shops) {
                ...
                const response = await MailController.sendMailToCustomer(s, subject, html)
                responses.push(response)
            }
